            ...
            <form action="{{ route('procesos.facturas.store') }}" method="POST" class="modern-form">
                ...
                <div class="form-grid">
                        ...
                        <select name="id_pag" id="id_pag_select" required onchange="populateServicios()">
                            ...
                            @foreach($pagos as $pago)
                                ...
                                <option value="{{ $pago->id_pag }}" data-servicios="{{ json_encode($serviciosJson) }}">
                                    Pago #{{ $pago->id_pag }} - {{ $pago->paciente->nom_pac ?? 'N/A' }} (${{ number_format($pago->mnt_pag, 2) }})
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="num_fac" value="{{ $nextNumFac }}" readonly style="background: #f0f0f0; cursor: not-allowed;">
                    </div>
                    ...
                    <div class="form-group" style="grid-column: span 3;">
                        ...
                        <div id="servicios-container" style="background: #fdfdfd; padding: 15px; border: 1px dashed #ccc; border-radius: 8px;">
                            <p id="placeholder-text" style="color: #999; font-style: italic;">Seleccione un pago para cargar los detalles...</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-actions">
                    ...
                    <button type="button" class="btn-secondary" onclick="addServicioRow()" style="margin-left: 10px;">+ Agregar Extra</button>
                </div>
            </form>
            @else
                <div class="factura-details" style="background: white; padding: 20px; border-radius: 8px; border: 1px solid #ddd;">
                    ...
                    <div style="text-align: right; margin-top: 15px;">
                        ...
                        <p style="font-size: 1.2rem; color: #1e3a8a;"><strong>Total:</strong> ${{ number_format($facturaEdit->ttl_fac, 2) }}</p>
                    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <form action="{{ route('procesos.facturas.destroy', $item->id_fac) }}" method="POST"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn-icon delete" onclick="return confirm('¿Eliminar esta factura?')">
